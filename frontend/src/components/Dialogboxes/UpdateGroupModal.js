...
const UpdateGroupModal = ({ open, close }) => {
  ...
  return <div className="dialog" style={{ position: "absolute", left: "0", zIndex: "20", display: open ? "block" : "none" }}>
    <div className="dialogbox">
      ...
      {!adduserbox?(<><div className='userdetails disflex' style={{ height:isMobile?"auto":"20vh"}}>
        {selectedchat.users.map((u) => {
          return (<div key={u._id} className='contactCard disflex' style={{ height: "3vh", justifyContent: "flex-start",padding:isMobile?"1em 0":"" }}>
      ...
      <form className='RegLegForm disflex' style={{ width: "100%",paddingLeft:"0", flexDirection: "column",height:"auto" }} autoComplete='off'>
        <div className='registerInput' style={{ width: '90%', border: "none" }}><input style={{ border: `1px solid ${appcolor}` }} className='RegInput' type="text" placeholder='Group name' name="name" onChange={(e) => { setGroupname(e.target.value) }} />
          <button className="lbtn h100 disflex" onClick={updateName} style={{ width: '30%', fontSize: "1.1rem", margin: "0 .4em",padding:'.6em .3em' }}>{renameLoading ? <Spinner simple={true} big={false} /> : "Update"}</button></div>
          <div className='registerInput' style={{ width: '90%', border: "none", marginTop:isMobile?"0":"" }}><button className='lbtn w100' style={{padding:isMobile?".6em 0":""}} onClick={()=>{setAdduserbox(!adduserbox)}}>Add Member</button></div>
      </form></>):
      ...
      {selectedchat.groupAdmin._id!==user._id && <div className='w100 disflex' style={{ justifyContent: "flex-end" }}><button className='lbtn' style={{ width: "100px", margin:isMobile?"0em 1em .6em 1em": ".7em 1em", backgroundColor: "red" }}>Leave</button></div>}
    </div>
  </div>;
};
